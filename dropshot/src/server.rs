...
pub trait Middleware<C: ServerContext>: Send + Sync + Debug {
    async fn handle(
        ...
        next: fn(
            ...
            Box<
                dyn Future<Output = Result<Response<Body>, HandlerError>>
                    + Send,
            >,
        >,
...
impl<C: ServerContext> HttpServerStarter<C> {
    ...
    pub fn new(
        ...
        HttpServerStarter::new_with_tls(config, api, private, middleware, None)
    }
    ...
    pub fn new_with_tls(
        ...
        ServerBuilder::new(api, private, middleware)
            .config(config.clone())
            .tls(tls)
            .build_starter()
    ...
    fn new_internal(
        ...
        for (path, method, _) in app_state.router.endpoints(None) {
            trace!(method = method, path = path, "registered endpoint");
        }

        let http_acceptor = HttpAcceptor { tcp };
    ...
    pub fn start(self) -> HttpServer<C> {
            ...
            match tls_acceptor {
                Some(tls_acceptor) => {
                    let mut https_acceptor =
                        HttpsAcceptor::new(tls_acceptor, http_acceptor);
                    loop {
                        tokio::select! {
                            ...
                            _ = &mut rx => {
                                trace!("beginning graceful shutdown");
                                break;
                            }
                        }
                    }
        ...
        let probe_registration = match usdt::register_probes() {
            ...
            Err(e) => {
                let msg = e.to_string();
                trace!("failed to register DTrace USDT probes: {}", msg);
...
async fn http_request_handle<C: ServerContext>(
    ...
    let uri = request.uri();
    let found_version = server.version_policy.request_version(&request)?;
    ...
    let mut response = match server.config.default_handler_task_mode {
        ...
        HandlerTaskMode::Detached => {
            ...
            match rx.await {
                Ok(result) => result?,
                Err(_) => {
                    tracing::error!("handler panicked; propogating panic");
